...
Michelson’s estimate of the error is less than the true error. The
difference betweeen Michelson’s estimate and the actual value is
151.542, while Michelson estimated the plus minus bounds to be 51.
...
- The means of both the real and simulated graphs stay close the same
  centerline at x = 299950.

- Both graphs seem to reach very similar values for velocity at the end
  of June

- The means of both graphs seem to stay within the bounds of 299900
  m/s - 300000 m/s (with the exception of a few outliers)

Differences

- There generally seem to be higher peaks and lower troughs in the real
  data graph, especially toward the beginning of June.

- There seems to be more data points for the simulated graph compared to
  the real graph.

- There’s more variation in the mean toward the beginning of June for
  the real data, whereas there’s more variation in the mean toward the
  end of June for the simulated data.
...
``` r
df_q2 %>% 
  ggplot(aes(x=Date, y=VelocityVacuum, colour = Temp)) +
  geom_point()
```
...
``` r
df_q2 %>% 
  ggplot(aes(x=Distinctness, y=VelocityVacuum, colour = Temp)) +
  geom_point()
```
...
- There are the least number of data points for distinctness 1.
- The lowest velocity in vacuum values tend to have low temperatures as
  well.
- Distinctness levels 2 and 3 have more high temperature values compared
  level 1, though level 3 has the most high temperature values.
- Most distinctness level 1 values were founded between June 13 and June
  20, whereas level 2 and 3 are a lot more varied regarding the date the

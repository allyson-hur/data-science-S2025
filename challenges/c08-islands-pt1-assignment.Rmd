...
-   Are there any sources of *real* uncertainty in the percent occupied you calculated?
    -   Yes, because islanders are born, die, and move around. If islanders move around, the number of occupied houses will change overtime.
-   Are there any sources of *erroneous* uncertainty in the percent occupied you calculated?
    -   I rounded to the nearest hundredth, so this rounding could have an effect.
...
-   Is this sample representative of *all* houses in Helvig? Why or why not?
    -   No because this sample only includes the first 25 houses in Helvig. To be representative of all houses, we'll need to have all houses in the dataset.
...
-   Which sample---sequential or random---is more *representative* of all homes Helvig? Why?
    -   Random
    -   Random sampling works better. There might be factors we don't know about that are particular to the first 25 houses in Helvig. For example, perhaps the families in the first 25 houses would've been on Helvig for the longest, and therefore would have larger family networks. There's also Helvig Hall and a hotel in the first 25 buildings, which are unique in terms of occupants.
...
-   Is there any reason that people with the same last name might tend to *live near each other*?
    -   People with the same last name can mean that they're married or are related, in which case they probably live near each other because they're family.
...
-   Note that we used the *random* sample with the bootstrap procedure in this task. Could we use the bootstrap to make a confidence interval using the sequential sample (`df_sample_seq`) that would be representative of all of Helvig? Why or why not?
    -   No
    -   There will be uncertainty if we use the sequential sample. Since the sequential sample is a specific subset of the houses, and people who are related tend to live near each other, the sample might only include a few families, which is not representative of Helvig.
...
-   Experiment: give a drink, then have them perform a mental and coordination test

...
df_psaap %>% 
  ggplot(aes(x = x, y = T_norm, colour = factor(idx))) +
  geom_point() +
  geom_line()
```
...
- Which model is *most accurate*? Which is *least accurate*?
  - fit_cheat is the most accurate according to the mean error squared
    since it has the lowest value.
  - fit_cheat is also the most accurate according to the coefficient of
    determination because it’s the highest value.
  - fit_nonphysical is the least accurate according to mse
...
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - idx is a metadata variable that doesn’t have a physical meaning.
...
- Which columns are excluded in the model formula above? What categories
  ...
  the model?
  - The columns excluded in the model above are idx, avg_q, avg_T, and
    rms_T. It’s important to leave these quantities out since none of
    them are input variables.
...
- What is the regression coefficient for `x`? What about the regression
  coefficient for `T_f`?
  - The regression coefficient for x is 1.018, while the regression
...
- Note that literally *all* of the inputs above have *some* effect on
  ...
  - Just because an input has some effect on the output doesn’t mean
    that it would be deemed a statistically significant difference.
...
- What fraction of validation cases lie within the intervals you
  ...
  - 93%, which is higher than the pr_level.
- What interval for `T_norm` would you recommend the design team to plan
  around?
  - I would recommend an interval of 1.3 to 1.6.
- Are there any other recommendations you would provide?
  - Additional inputs may be added to the model, which could vary the
    results.
  - Other variables like d_p and rho_p, could be included.
